...
public class SocialMediaAdminImpl extends RemoteServiceServlet implements SocialMediaAdmin {
	...
	public Kommentar findKommentarById(int id) {
		return this.kMapper.findKommentarById(id);
	}
	...
	public void deleteBeitrag(Beitrag b) {
		Vector<Kommentar> k = findKommentareByBeitrag(b.getId());
		for (Kommentar kommentar : k) {
			this.kMapper.delete(kommentar);
		}

		Vector<Like> l = findLikesByBeitrag(b.getId());
		for (Like like : l) {
			this.lMapper.delete(like);
		}
