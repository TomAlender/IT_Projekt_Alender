...
public class FremdePinnwand extends VerticalPanel {
						...
						public void onSuccess(Vector<Kommentar> result) {
							...
							if (result.size() == 0) {
								...
								for (Kommentar kommentar : result) {
									...
									ft.setWidget(ft.getRowCount(), 0, new KommentarForm(kommentar, currentNutzer, pinn));
					...
					neuKommBtn.addClickHandler(new ClickHandler() {
						...
						public void onClick(ClickEvent event) {
							if (kommtextarea.getText().equals("")) {
								...
								socialMedia.createKommentar(currentNutzer, beitragId, kommtextarea.getText(),
	...
	public class FindKommCallBack implements AsyncCallback<Vector<Kommentar>> {
		...
		public void onSuccess(Vector<Kommentar> result) {
			...
			for (Kommentar kommentar : result) {
				...
				kommdatum.setText(
						DateTimeFormat.getFormat("dd-MM-yyyy HH:mm:ss").format(kommentar.getErstellungszeitpunkt()));
	...
	public class DeabonnierenCLickhandler implements ClickHandler {
		...
		@Override
		public void onClick(ClickEvent event) {
			socialMedia.deleteAbonnement(Integer.parseInt(Cookies.getCookie("id")), nickname,
